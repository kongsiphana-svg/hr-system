...
@section('content')
    ...
    <div style="background-color: #f3f5f9; min-height: 100%; padding: 24px;">
        {{-- Header with navigation --}}
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-3xl font-bold text-slate-800">My Schedule</h2>
            ...
            <div class="flex items-center gap-3">
                <a
                    href="{{ route('employee.schedule', ['week' => $weekStart->copy()->subWeek()->toDateString()]) }}"
                ...
                <a
                    href="{{ route('employee.schedule', ['week' => now()->toDateString()]) }}"
                    class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-500 shadow-sm hover:text-slate-700"
                >
                    Today
                </a>
                <a
                    href="{{ route('employee.schedule', ['week' => $weekStart->copy()->addWeek()->toDateString()]) }}"
                    class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-500 shadow-sm hover:text-slate-700"
                >
                    Next →
                </a>
        ...
        <div class="mb-8 overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            {{-- Employee Row --}}
            <div class="grid min-h-[120px]" style="grid-template-columns: 180px repeat(7, 1fr) 80px;">
                {{-- Employee Info --}}
                <div class="flex items-center space-x-3 border-r border-slate-100 p-3">
                    @if ($employee && $employee->avatar_url)
                        <img src="{{ $employee->avatar_url }}" alt="{{ $employee->name }}" class="h-9 w-9 rounded-full object-cover shrink-0">
                    @else
                    ...
                    @endif
                    <div class="min-w-0">
                        <p class="text-xs font-bold text-slate-800 truncate">{{ $employee ? $employee->name : 'Me' }}</p>
                        <p class="text-[10px] text-slate-400 truncate">{{ $employee ? ($employee->department ?: '—') : '—' }}</p>
                ...
                @for ($dayIndex = 0; $dayIndex < 7; $dayIndex++)
                    <div class="relative p-1.5 border-r border-slate-50 min-h-[100px]">
                        @if (isset($employeeShifts[$dayIndex]))
                            @php $shift = $employeeShifts[$dayIndex]; @endphp
                ...
                <div class="flex items-center justify-center p-2">
                    <button
                        ...
                        data-employee-name="{{ $employee ? $employee->name : 'Me' }}"
                        data-employee-dept="{{ $employee ? ($employee->department ?: '—') : '—' }}"
                        data-employee-initials="{{ $employee ? $employee->initials() : '?' }}"
                        data-employee-avatar="{{ $employee ? ($employee->avatar_url ?? '') : '' }}"
                        data-fixed-start="{{ $employee ? ($employee->fixed_start_time ?? '') : '' }}"
                        data-fixed-end="{{ $employee ? ($employee->fixed_end_time ?? '') : '' }}"
                        data-work-days="{{ json_encode($employee ? ($employee->fixed_work_days ?? []) : []) }}"
                        data-shifts="{{ json_encode($employeeShifts) }}"
